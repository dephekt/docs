...
[NOTE]
====
. To enter the password of the service account via the console, the `password` argument, the `password-file` option and the `password-from-service-account` option must not be set.

. To use the password for the service account from the mount point settings, use the `password-from-service-account` option.

. If you use the `password` argument together with the `password-file` option, the `password-file` option takes precedence over the `password` argument.
====
